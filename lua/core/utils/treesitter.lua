...
M.find_node_by_type = function(node, type)
	while node do
...
M.find_child_by_type = function(node, type)
	local child_id = 0
...
M.find_child_node_name_by_type = function(node_type, child_type)
	...
	local method_node = M.find_node_by_type(current_node, node_type)
	if not method_node then
		return nil
	end

	local identifier = M.find_child_by_type(method_node, child_type)
